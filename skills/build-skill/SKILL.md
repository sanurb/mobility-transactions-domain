...
Place in `skills/` (this repo), `.opencode/skills/` (project), or `~/.config/opencode/skills/` (global).
...
| Priority | Location |
|----------|----------|
| 1 | **`skills/<name>/`** (this repo: single source of truth at repo root) |
| 2 | `.opencode/skills/<name>/` (project) |
| 3 | `~/.config/opencode/skills/<name>/` (global) |
| 4 | `.claude/skills/<name>/`, `.codex/skills/<name>/`, `.gemini/skills/<name>/` (created by `skills/setup.sh` symlinks) |

In this repo, skills live in `skills/`. Run `./skills/setup.sh` to symlink into tool-specific dirs. Discovery walks up from CWD to git root. First-wins for duplicate names.
